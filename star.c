...
static int __star_resume(struct net_device *ndev)
{
	...
	starDev->linkUp = (val & STAR_PHY_CTRL1_STA_LINK)?1UL:0UL;
	STAR_MSG(STAR_ERR, "resume back PHY_CTRL1=%x\n", val);
	STAR_MSG(STAR_ERR, "PHY REG1=%x\n", (int)StarMdcMdioRead(starDev, starPrv->phy_addr, 0x01));
	if (starDev->linkUp)
    ...
    switch (cmd) {
        ...
        case SIOC_SET_WOP_CMD:
		{
            struct ioctl_wop_para_cmd *wop_cmd =
									(struct ioctl_wop_para_cmd *)req->ifr_data;
            star_setWOPProtocolPort(starDev, wop_cmd->protocol_type,
									wop_cmd->port_array, wop_cmd->port_count);
            break;
        }
